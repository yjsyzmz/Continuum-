...
scripts/
└─ generate-continuum-icons.ps1
```

当前图标源文件是 `assets/brand/continuum-icon.svg`。Chrome Web Store 128x128 图标和扩展内多尺寸图标由 `scripts/generate-continuum-icons.ps1` 生成，确保 128x128 PNG 中实际图标主体为 96x96，四周保留 16px 透明内边距。
